...
public class DBCustomHTTPProtocol: URLProtocol {
  public static var ignoredHosts = [String]()
  @available(*, deprecated, renamed: "ignoredHosts") static var blacklistedHosts: [String] {
    get {
      ignoredHosts
    }
    set {
  ...
  var currentRequest: DBRequestModel?
  ...
  override public class func canInit(with request: URLRequest) -> Bool {
    guard DBCustomHTTPProtocol.shouldHandleRequest(request) else { return false }
    return DBCustomHTTPProtocol.property(forKey: Constants.RequestHandledKey, in: request) == nil
  }
  ...
  override public func startLoading() {
    ...
    DBCustomHTTPProtocol.setProperty(true, forKey: Constants.RequestHandledKey, in: newRequest)
    sessionTask = session?.dataTask(with: newRequest as URLRequest)
    sessionTask?.resume()

    currentRequest = DBRequestModel(request: newRequest, session: session)
    DBStorage.shared.saveRequest(request: currentRequest)
  }
  ...
  private class func shouldHandleRequest(_ request: URLRequest) -> Bool {
    ...
    if DBCustomHTTPProtocol.ignoredHosts.contains(where: { host.hasSuffix($0) }) {
      return false
    }
    return true
  }
...
extension DBCustomHTTPProtocol: URLSessionDataDelegate {
