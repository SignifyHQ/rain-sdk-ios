...
extension NSCardDataTests {
  ...
  func test_postApplePayToken_shouldReturnSuccessResponse() async {
    ...
    // When calling postApplePayToken function on the repository and it should return a success response
    await expect {
      try await self.repository.postApplePayToken(
        sessionId: self.mockValidNetSpendSessionID,
        cardId: self.mockExistCardID,
        bodyData: self.mockApplePayToken
      ).activationData
    }
    // Then the activationData received matches our expectation
    .to(equal(mockPostApplePayTokenResponse.activationData))
  }
  
  /// Test postApplePayToken functionality when it encounters an API error
  func test_postApplePayToken_shouldThrowError() async {
    // Given a mock error which will be thrown
    self.api.postApplePayTokenSessionIdCardIdBodyDataThrowableError = expectedThrowableError
    
    // When calling postApplePayToken function on the repository
    await expect {
      try await self.repository.postApplePayToken(
        sessionId: self.mockValidNetSpendSessionID,
        cardId: self.mockExistCardID,
  ...
  /// Test postApplePayToken functionality with invalid netspendSessionID when it encounters an API error
  func test_postApplePayToken_withInvalidNetSpendSessionID_shouldThrowError() async {
    // Given the expected API behaviour
    configureAPIBehaviour()
  ...
  /// Test postApplePayToken functionality with not exist CardID when it encounters an API error
  func test_postApplePayToken_withNotExistCardID_shouldThrowError() async {
    // Given the expected API behaviour
    configureAPIBehaviour()
...
private extension NSCardDataTests {
  ...
  func configureAPIBehaviour() {
    api.postApplePayTokenSessionIdCardIdBodyDataClosure = { netspendSessionID, cardID, bodyData async throws in
      ...
      default:
        return self.mockPostApplePayTokenResponse
      }
    }
  }
}
