...
extension NSCardDataTests {
  ...
  func test_unlockCard_shouldReturnSuccessResponse() async {
    ...
    await expect {
      try await self.repository.unlockCard(
  ...
  /// Test unlockCard functionality when it encounters an API error
  func test_unlockCard_shouldThrowError() async {
    // Given a mock error which will be thrown
    self.api.unlockCardCardIDSessionIDThrowableError = expectedThrowableError
    
    // When calling unlockCard function on the repository
    await expect {
      try await self.repository.unlockCard(
        cardID: self.mockExistCardID,
        sessionID: self.mockValidNetSpendSessionID
  ...
  /// Test unlockCard functionality with invalid netspendSessionID when it encounters an API error
  func test_unlockCard_withInvalidNetSpendSessionID_shouldThrowError() async {
    // Given the expected API behaviour
    configureAPIBehaviour()
  ...
  /// Test unlockCard functionality with not exist CardID when it encounters an API error
  func test_unlockCard_withNotExistCardID_shouldThrowError() async {
    // Given the expected API behaviour
    configureAPIBehaviour()
    // When calling unlockCard function on the repository
    await expect {
...
private extension NSCardDataTests {
  ...
  func configureAPIBehaviour() {
    api.unlockCardCardIDSessionIDClosure = { cardID, netspendSessionID async throws in
