...
extension NSCardDataTests {
  ...
  func test_orderPhysicalCard_shouldReturnSuccessResponse() async {
    ...
    // When calling orderPhysicalCard function on the repository and it should return a success response
    await expect {
  ...
  func test_orderPhysicalCard_shouldThrowError() async {
    ...
    self.api.orderPhysicalCardAddressSessionIDThrowableError = expectedThrowableError
    
    // When calling orderPhysicalCard function on the repository
    await expect {
      try await self.repository.orderPhysicalCard(
        address: self.mockAddressOrderCard,
        sessionID: self.mockValidNetSpendSessionID
  ...
  /// Test orderPhysicalCard functionality with invalid netspendSessionID when it encounters an API error
  func test_orderPhysicalCard_withInvalidNetSpendSessionID_shouldThrowError() async {
    // Given the expected API behaviour
    configureAPIBehaviour()
...
private extension NSCardDataTests {
  ...
  func configureAPIBehaviour() {
    api.orderPhysicalCardAddressSessionIDClosure = { parameter, netspendSessionID async throws in
