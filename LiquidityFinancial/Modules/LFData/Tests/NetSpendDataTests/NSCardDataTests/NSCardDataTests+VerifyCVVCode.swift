...
extension NSCardDataTests {
  ...
  func test_verifyCVVCode_shouldReturnSuccessResponse() async {
    ...
    // When calling verifyCVVCode function on the repository and it should return a success response
    await expect {
      try await self.repository.verifyCVVCode(
        verifyRequest: self.mockVerifyCode,
        cardID: self.mockExistCardID,
        sessionID: self.mockValidNetSpendSessionID
      ).id
    }
    // Then the id received matches our expectation
    .to(equal(self.mockVerifyCVVCodeResponse.id))
  }
  
  /// Test verifyCVVCode  functionality when it encounters an API error
  func test_verifyCVVCode_shouldThrowError() async {
    // Given a mock error which will be thrown
    self.api.verifyCVVCodeVerifyRequestCardIDSessionIDThrowableError = expectedThrowableError
    
    // When calling verifyCVVCode function on the repository
    await expect {
      try await self.repository.verifyCVVCode(
        verifyRequest: self.mockVerifyCode,
        cardID: self.mockExistCardID,
  ...
  /// Test verifyCVVCode functionality with invalid netspendSessionID when it encounters an API error
  func test_verifyCVVCode_withInvalidNetSpendSessionID_shouldThrowError() async {
    // Given the expected API behaviour
    configureAPIBehaviour()
  ...
  /// Test verifyCVVCode functionality with not exist CardID when it encounters an API error
  func test_verifyCVVCode_withNotExistCardID_shouldThrowError() async {
    // Given the expected API behaviour
    configureAPIBehaviour()
...
private extension NSCardDataTests {
  ...
  func configureAPIBehaviour() {
    api.verifyCVVCodeVerifyRequestCardIDSessionIDClosure = { parameter, cardID, netspendSessionID async throws in
      ...
      default:
        return self.mockVerifyCVVCodeResponse
      }
    }
  }
}
