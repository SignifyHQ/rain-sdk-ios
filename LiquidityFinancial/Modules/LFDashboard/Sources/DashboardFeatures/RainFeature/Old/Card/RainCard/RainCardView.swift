  ...
  var body: some View {
    ZStack(alignment: .bottom) {
      ZStack(alignment: .topLeading) {
        (isFrntCard ? GenImages.Images.frntCardEmpty.swiftUIImage : GenImages.Images.emptyCard.swiftUIImage)
          .resizable()
          .aspectRatio(contentMode: .fit)
        
        Text(viewModel.cardModel.cardType.title)
          .font(Fonts.semiBold.swiftUIFont(size: Constants.FontSize.small.value))
          .foregroundColor(isFrntCard ? .black : Colors.contrast.swiftUIColor)
          .padding(.top, 15)
          .padding(.leading, 10)
      }
      ...
      physicalTitleView
        .opacity((viewModel.cardModel.cardType != .physical || isFrntCard) ? 0 : 1)
    }
