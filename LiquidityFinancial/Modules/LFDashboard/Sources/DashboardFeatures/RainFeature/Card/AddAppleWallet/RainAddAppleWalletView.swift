...
struct RainAddAppleWalletView: View {
  @StateObject
  private var viewModel: RainAddAppleWalletViewModel
  
  init(viewModel: RainAddAppleWalletViewModel) {
  ...
  var body: some View {
    content
      ...
      .sheet(isPresented: $viewModel.isShowApplePay) {
        RainApplePayViewController(
          viewModel: RainApplePayViewModel(cardModel: viewModel.cardModel)
        ) {
          viewModel.onFinish()
        }
      }
...
private extension RainAddAppleWalletView {
