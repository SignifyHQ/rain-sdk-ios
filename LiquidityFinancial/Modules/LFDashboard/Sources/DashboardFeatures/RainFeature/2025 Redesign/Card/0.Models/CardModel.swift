...
struct CardModel: Identifiable, Hashable, Observable {
  let id: String
  ...
  init(
    ...
    id = order.id
    cardType = .physical
    cardStatus = CardStatus(rawValue: order.status.lowercased()) ?? .canceled
  ...
  var updatedAtDate: Date? {
    if let updatedAt = updatedAt,
       let format = LiquidityDateFormatter.getDateFormat(from: updatedAt),
       let date = format.parseToDate(from: updatedAt) {
      return date
    }
    return nil
  }
