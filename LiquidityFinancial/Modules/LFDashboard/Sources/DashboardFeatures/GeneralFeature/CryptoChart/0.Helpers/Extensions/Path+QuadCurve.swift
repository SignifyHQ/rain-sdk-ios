...
extension Path {
  ...
  static func drawGridXLine(index: Int, count: Int, in rect: CGRect) -> Path {
    var path = Path()

    let normalisedSpacing = 1.0 / CGFloat(count - 1)
    let startPoint = CGPoint(x: normalisedSpacing * CGFloat(index) * rect.width, y: 0)
    let endPoint = CGPoint(x: normalisedSpacing * CGFloat(index) * rect.width, y: rect.height)
    path.move(to: startPoint)
    path.addLine(to: endPoint)
  ...
  static func drawGridXLine(xOffset: Double, in rect: CGRect) -> Path {
    var path = Path()

    let startPoint = CGPoint(x: xOffset * rect.width, y: 0)
    let endPoint = CGPoint(x: xOffset * rect.width, y: rect.height)
    path.move(to: startPoint)
    path.addLine(to: endPoint)
...
extension CGPoint {
  ...
  func point(to: CGPoint, t: CGFloat, control: CGPoint) -> CGPoint {
    let x = CGPoint.value(x: x, y: to.x, t: t, c: control.x)
    let y = CGPoint.value(x: y, y: to.y, t: t, c: control.y)
  ...
  func point(to: CGPoint, t: CGFloat, control1: CGPoint, control2: CGPoint) -> CGPoint {
    let x = CGPoint.value(x: x, y: to.x, t: t, control1: control1.x, control2: control2.x)
    let y = CGPoint.value(x: y, y: to.y, t: t, control1: control1.y, control2: control2.x)
