...
struct NSAddAppleWalletView: View {
  @StateObject
  private var viewModel: NSAddAppleWalletViewModel
  
  init(viewModel: NSAddAppleWalletViewModel) {
  ...
  var body: some View {
    content
      ...
      .sheet(isPresented: $viewModel.isShowApplePay) {
        NSApplePayViewController(
          viewModel: NSApplePayViewModel(cardModel: viewModel.cardModel)
        ) {
          viewModel.onFinish()
        }
      }
...
private extension NSAddAppleWalletView {
