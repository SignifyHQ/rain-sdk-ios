...
public struct DonationsDisclosureView: View {
  ...
  private var alert: some View {
    PopupAlert {
      VStack(spacing: 16) {
        ...
        .lineSpacing(1.33)
        .multilineTextAlignment(.center)
        FullSizeButton(title: LFLocalizable.Button.Ok.title, isDisable: false, type: .primary, gradientColor: gradientColor) {
          showPopup = false
        }
      }
    }
  ...
  var gradientColor: [Color] {
    [
      ModuleColors.Gradients.Button.gradientButton0.swiftUIColor,
      ModuleColors.Gradients.Button.gradientButton1.swiftUIColor
    ]
  }
}
