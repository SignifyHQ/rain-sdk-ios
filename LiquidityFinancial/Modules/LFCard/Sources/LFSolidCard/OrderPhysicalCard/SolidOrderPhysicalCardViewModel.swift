...
@MainActor
final class SolidOrderPhysicalCardViewModel: OrderPhysicalCardViewModelProtocol {
  ...
  init(coordinator: BaseCard.BaseCardDestinationObservable, onOrderSuccess: ((BaseCard.CardModel) -> Void)?) {
...
// MARK: - API Handle
extension SolidOrderPhysicalCardViewModel {
...
extension SolidOrderPhysicalCardViewModel {
  func onClickedEditAddressButton(shippingAddress: Binding<ShippingAddress?>) {
    let destinationView = ShippingAddressView(
      viewModel: SolidShippingAddressViewModel(shippingAddress: shippingAddress)
    )
