...
class IssuerExtensionHandler: PKIssuerProvisioningExtensionHandler {
  ...
  override func passEntries(
    ...
    Task {
      ...
      for card in cards {
        ...
        TokenizationReceiptStore.shared.save(responseData.tokenizationReceipt, for: card.id)
  ...
  override func generateAddPaymentPassRequestForPassEntryWithIdentifier(
    ...
    Task {
      self.logger.info("WalletExtension: Generating add pass request for identifier \(identifier, privacy: .public)")
      
      guard let tokenizationReceipt = TokenizationReceiptStore.shared.retrieve(for: identifier)
      else {
        self.logger.error("WalletExtension: Missing tokenization receipt for identifier \(identifier, privacy: .public)")
        completion(nil)
      ...
      do {
        ...
        guard responseData.isValid(),
              let request = responseData.addPaymentPassRequest
        else {
          self.logger.error("WalletExtension: Invalid response data for identifier \(identifier, privacy: .public)")
          completion(nil)
          
          return
        }
        
        self.logger.info("WalletExtension: Successfully generated add pass request for \(identifier, privacy: .public)")
        completion(request)
      } catch {
        self.logger.error("WalletExtension: Tokenization error for \(identifier, privacy: .public): \(error.localizedDescription, privacy: .public)")
        completion(nil)
      }
    }
  }
  
  // TODO(Volo): Improve this and make reusable to support other API calls
  func getActiveCards(
  ) async -> [ActiveCard]? {
