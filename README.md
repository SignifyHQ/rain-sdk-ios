...
- **Rain wallet** — Register a `RainProvider` (zero configuration — the backend identity is embedded); the SDK runs login-code auth over email or SMS (`sendLoginCode(to:)` / `confirmLoginCode`) and passkeys (`loginWithPasskey` / `signUpWithPasskey` / `addPasskey`), provisions one wallet with EVM + Solana accounts on first login, manages the session, and supports on-device key export (`exportRecoveryPhrase` / `exportPrivateKey`). See [rain-wallet-ios/README.md](rain-wallet-ios/README.md).
